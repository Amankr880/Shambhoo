...
@section('content')
    @include('users.partials.header', [
        'title' => 'Admin Profile',
        'description' => $admin[0]['admin_name']
    ])  
    <div class="container-fluid mt--7">
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        <form action="{{url('updateadmin')}}" enctype="multipart/form-data" method="post">
                            @csrf
                            ...
                            @elseif($key=='icon')
                             <div class="row form-group d-flex align-items-center">
                                ...
                                <div class="col d-flex align-items-center gx-4">
                                    ...
                                    <img src="{{asset('storage/assets/img/admin_icons/'.$value)}}" style="max-height: 100px;max-width:100px;margin-right: 20px;"><div>Change icon:<br><br><input type="file" name="icon_file"></div>
                                </div>
                            </div>
                            @elseif($key=='parent_admin')
                             <div class="row form-group d-flex align-items-center">
                                <div class="col-3">
                                    <label class="form-control-label" for="input-name">{{$key}}</label>
                                </div>
                                <div class="col">
                                    <select class="form-select" name="{{$key}}">
                                        ...
                                        <option value="{{$parent_categories['id']}}" @if($value==$parent_categories['id'])selected @endif>{{$parent_categories['admin_name']}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
