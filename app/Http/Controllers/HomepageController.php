...
class HomepageController extends Controller
{
    ...
    {
        $shopDetails = Vendor::where([['pincode','=',$request->pincode],['status','>',1],['visibility','=',1]])->select('*',DB::raw("CONCAT('storage/assets/img/logo/',logo_image) AS logo_image,CONCAT('storage/assets/img/header/',header_image) AS header_image"))->orderBy('status','desc')->get();  
        if($shopDetails)
        {
